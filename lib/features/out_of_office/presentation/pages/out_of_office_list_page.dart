...
class _OutOfOfficeListPageState extends State<OutOfOfficeListPage> {
  ...
  @override
  void didUpdateWidget(OutOfOfficeListPage oldWidget) {
        ...
        return Scaffold(
          ...
          floatingActionButton: c.isOwner
              ...
              : FloatingActionButton(
                  onPressed: () async {
                    await context.push('/out-of-office-add');
                    if (mounted) {
                      c.fetchOutOfOffices();
                    }
                  },
...
class _OutOfOfficeCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasButtons = controller.isOwner && item.isPending;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow('Tanggal Mulai', item.startDate != null ? DateFormat('dd MMMM yyyy', 'id_ID').format(item.startDate!) : '-'),
            const SizedBox(height: 6),
            _buildDetailRow('Tanggal Selesai', item.endDate != null ? DateFormat('dd MMMM yyyy', 'id_ID').format(item.endDate!) : '-'),
            const SizedBox(height: 6),
